...
public class Unit : MonoBehaviour
{
	...
	private void TypeSet()
	{
		...
		switch (type) // 유닛 타입별로 적군 탐색 레이저 길이 조정
		{
			case UnitType.Sword:
				distance = 0.7f;
				SGSoundCheck = GameManager.instance.SoundManager.GetComponent<SoundManager>().AtkSword;
				break;
			case UnitType.Range:
				distance = 2.5f;
			...
			case UnitType.Guard:
				distance = 0.7f;
				SGSoundCheck = GameManager.instance.SoundManager.GetComponent<SoundManager>().AtkGuard;
				break;
	...
	IEnumerator AttackDelay()
	{
		if (type == UnitType.Wizard) //클래스 wizard의 경우 레이더에 걸린 상대의 위치에 파티클 재생
		{
			...
			if(!GameManager.instance.SoundManager.GetComponent<SoundManager>().AtkMagic.isPlaying)
				GameManager.instance.SoundManager.GetComponent<SoundManager>().AtkMagic.Play();
		}
		if (type == UnitType.Range) // 공격시 앞으로 나가는 화살 생성
		{
			Instantiate(Arrow, new Vector3(transform.position.x, 0.3f, 0), Quaternion.identity);
			if(!GameManager.instance.SoundManager.GetComponent<SoundManager>().AtkRange.isPlaying)
				GameManager.instance.SoundManager.GetComponent<SoundManager>().AtkRange.Play();
		}
		else
		{
			...
			if(SGSoundCheck != null&&!SGSoundCheck.isPlaying)
				SGSoundCheck.Play();
		}
